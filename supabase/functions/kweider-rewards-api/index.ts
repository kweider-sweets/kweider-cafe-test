...
const createPinSalt = ()=>createRawToken().slice(0, 24);
const pinPepper = ()=>{
  const pepper = Deno.env.get("KWEIDER_PIN_PEPPER") || Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") || "";
  if (!pepper) {
    throw new ApiError(500, "pin_service_unavailable", "Secure PIN access is temporarily unavailable.");
  }
  return pepper;
};
const hmacSha256Hex = async (keyValue, message)=>{
...
const hashPin = async (pin, salt)=>hmacSha256Hex(pinPepper(), `${salt}:${pin}`);
...
const normaliseMemberPhone = async (admin, phone)=>{
  const { data, error } = await admin.rpc("kweider_normalize_phone", {
    p_phone: phone
  });
  if (error || !data) {
    throw new ApiError(400, "invalid_phone", "Use a valid UK number beginning 07 or +44.");
  }
  return String(data);
};
const issueMemberCardToken = async (admin, memberId)=>{
  const rawToken = createRawToken();
  const tokenHash = await sha256Hex(rawToken);
  const { error } = await admin.from("kweider_member_access_tokens").insert({
    member_id: memberId,
    token_hash: tokenHash
  });
  if (error) {
    console.error("Unable to issue member card token:", error);
    throw new ApiError(500, "token_creation_failed", "The membership card could not be secured.");
  }
  return rawToken;
};
const requireMemberAccess = async (admin, tokenValue)=>{
  const token = cleanText(tokenValue);
  if (token.length < 30) {
    throw new ApiError(401, "invalid_card_token", "The saved membership card is not valid.");
  }
  const tokenHash = await sha256Hex(token);
  const { data: accessToken, error } = await admin.from("kweider_member_access_tokens").select("id, member_id, revoked_at").eq("token_hash", tokenHash).maybeSingle();
  ...
  return {
    token,
    accessToken,
    memberId: String(accessToken.member_id)
  };
};
const memberSelect = `
  id,
...
const getSettings = async (admin)=>{
  const { data, error } = await admin.from("kweider_loyalty_settings").select("points_per_pound, points_per_reward, reward_value").eq("id", 1).single();
  if (error || !data) {
    console.error("Unable to read loyalty settings:", error);
    throw new ApiError(500, "settings_unavailable", "The loyalty settings are temporarily unavailable.");
  }
  return {
...
const formatTransactions = (transactions)=>(transactions ?? []).map((transaction)=>({
      ...
      purchaseAmount: transaction.purchase_amount === null ? null : Number(transaction.purchase_amount),
      rewardValue: transaction.reward_value === null ? null : Number(transaction.reward_value),
      receiptReference: transaction.receipt_reference,
      performedByName: transaction.performed_by_name,
      notes: transaction.notes,
      createdAt: transaction.created_at
    }));
const formatWelcomeCoffee = (coffee)=>{
  if (!coffee) return null;
  const expiresAt = coffee.expires_at || null;
  const expired = coffee.status === "available" && expiresAt && new Date(expiresAt).getTime() <= Date.now();
  return {
    id: coffee.id,
    ...
    purchaseAmount: coffee.purchase_amount === null || coffee.purchase_amount === undefined ? null : Number(coffee.purchase_amount),
    receiptReference: coffee.receipt_reference
  };
};
const loadMemberBundle = async (admin, memberId)=>{
  const { data: member, error: memberError } = await admin.from("kweider_members").select(memberSelect).eq("id", memberId).single();
  if (memberError || !member) {
    throw new ApiError(404, "member_not_found", "The membership could not be found.");
  }
  const { data: transactions, error: transactionError } = await admin.from("kweider_loyalty_transactions").select(transactionSelect).eq("member_id", memberId).order("created_at", {
    ascending: false
  }).limit(30);
  if (transactionError) {
    console.error("Unable to read transactions:", transactionError);
    throw new ApiError(500, "transaction_history_failed", "The membership history could not be loaded.");
  }
  const settings = await getSettings(admin);
  const nowIso = new Date().toISOString();
  const { data: rewardDefinitions, error: rewardDefinitionsError } = await admin.from("kweider_reward_definitions").select("code, threshold_points, reward_kind, title_en, title_ar, message_en, message_ar, percent_off, maximum_discount, validity_days, sort_order").eq("active", true).order("threshold_points", {
    ascending: true
  });
  if (rewardDefinitionsError) {
    console.error("Unable to read reward definitions:", rewardDefinitionsError);
    throw new ApiError(500, "reward_definitions_failed", "The rewards programme could not be loaded.");
  }
  const { data: rewards, error: rewardsError } = await admin.from("kweider_member_rewards").select("id, reward_code, cycle_number, status, issued_at, expires_at, redeemed_at").eq("member_id", memberId).order("issued_at", {
    ascending: false
  }).limit(20);
  if (rewardsError) {
    console.error("Unable to read member rewards:", rewardsError);
    throw new ApiError(500, "member_rewards_failed", "The member rewards could not be loaded.");
  }
  const { data: messages, error: messagesError } = await admin.from("kweider_member_messages").select("id, message_type, title_en, title_ar, body_en, body_ar, is_read, read_at, related_reward_id, created_at, expires_at").eq("member_id", memberId).lte("not_before", nowIso).order("created_at", {
    ascending: false
  }).limit(30);
  if (messagesError) {
    console.error("Unable to read member messages:", messagesError);
    throw new ApiError(500, "member_messages_failed", "The member messages could not be loaded.");
  }
  const { data: welcomeCoffee, error: welcomeCoffeeError } = await admin.from("kweider_welcome_coffees").select("id, status, issued_at, expires_at, redeemed_at, redeemed_by_name, purchase_amount, receipt_reference").eq("member_id", memberId).maybeSingle();
  if (welcomeCoffeeError) {
    console.error("Unable to read welcome coffee:", welcomeCoffeeError);
    throw new ApiError(500, "welcome_coffee_failed", "The welcome coffee could not be loaded.");
  }
  ...
  return {
    ...
    rewardDefinitions: (rewardDefinitions ?? []).map((definition)=>({
        ...
        percentOff: definition.percent_off === null ? null : Number(definition.percent_off),
        maximumDiscount: definition.maximum_discount === null ? null : Number(definition.maximum_discount),
    ...
    unreadMessageCount: formattedMessages.filter((message)=>!message.isRead).length
  };
};
const formatPinResetRequest = (request)=>request ? {
    id: request.id,
...
const latestActivePinReset = async (admin, memberId)=>{
  const { data, error } = await admin.from("kweider_pin_reset_requests").select("id, status, requested_at, expires_at, approved_at").eq("member_id", memberId).in("status", [
    "pending",
    "approved"
  ]).gt("expires_at", new Date().toISOString()).order("requested_at", {
    ascending: false
  }).limit(1).maybeSingle();
  if (error) {
    console.error("Unable to read PIN reset request:", error);
    throw new ApiError(500, "pin_reset_check_failed", "The PIN reset request could not be checked.");
  }
  return data || null;
};
const requestPinReset = async (payload, admin)=>{
  ...
  const { data: member, error: memberError } = await admin.from("kweider_members").select("id, member_code, first_name, status, access_pin_hash, access_pin_salt").eq("phone_e164", String(normalisedPhone)).maybeSingle();
  if (memberError) {
    console.error("Unable to find member for PIN reset:", memberError);
    throw new ApiError(500, "pin_reset_request_failed", "The PIN reset request could not be started.");
  }
  if (!member) {
  ...
  if (!member.access_pin_hash || !member.access_pin_salt) {
    throw new ApiError(409, "pin_not_set", "This card does not have a PIN yet. Open it on the device where it is already visible and create a PIN once.");
  }
  await admin.from("kweider_pin_reset_requests").update({
    status: "cancelled"
  }).eq("member_id", member.id).in("status", [
    "pending",
    "approved"
  ]);
  const { rawToken, expiresAt } = createPinResetToken();
  const tokenHash = await sha256Hex(rawToken);
  const { data: request, error: requestError } = await admin.from("kweider_pin_reset_requests").insert({
    member_id: member.id,
    token_hash: tokenHash,
    status: "pending",
    expires_at: expiresAt
  }).select("id, status, requested_at, expires_at, approved_at").single();
...
const checkPinReset = async (payload, admin)=>{
  ...
  const { data: request, error } = await admin.from("kweider_pin_reset_requests").select("id, member_id, status, requested_at, expires_at, approved_at, used_at").eq("token_hash", tokenHash).maybeSingle();
  if (error || !request) {
...
const completePinReset = async (payload, admin)=>{
  const resetToken = cleanText(payload.resetToken);
  ...
  const tokenHash = await sha256Hex(resetToken);
  const { data: request, error: requestError } = await admin.from("kweider_pin_reset_requests").select("id, member_id, status, expires_at, used_at").eq("token_hash", tokenHash).maybeSingle();
  if (requestError || !request) {
    throw new ApiError(404, "pin_reset_not_found", "This PIN reset request could not be found.");
  }
...
const createMembership = async (payload, admin)=>{
  ...
  const notificationConsent = payload.notificationConsent === true || payload.marketingConsent === true;
  ...
  if (birthday && !/^\d{4}-\d{2}-\d{2}$/.test(birthday)) {
    throw new ApiError(400, "invalid_birthday", "Enter the birthday in YYYY-MM-DD format.");
  }
  const settings = await getSettings(admin);
  const normalisedPhone = await normaliseMemberPhone(admin, phone);
  const pinSalt = pin ? createPinSalt() : null;
  const pinHash = pin && pinSalt ? await hashPin(pin, pinSalt) : null;
  const { data: existingMember, error: existingError } = await admin.from("kweider_members").select("id").eq("phone_e164", String(normalisedPhone)).maybeSingle();
  if (existingError) {
  ...
  if (existingMember) {
    throw new ApiError(409, "member_exists", "A membership already exists for this phone number.");
  }
  const rawToken = createRawToken();
  const tokenHash = await sha256Hex(rawToken);
  let memberId = null;
  try {
    const { data: member, error: memberError } = await admin.from("kweider_members").insert({
      ...
      notification_consent_at: notificationConsent ? new Date().toISOString() : null,
    ...
    if (memberError || !member) {
      ...
      throw new ApiError(500, "membership_creation_failed", "The membership could not be created.");
    }
    memberId = member.id;
    const { error: tokenError } = await admin.from("kweider_member_access_tokens").insert({
      member_id: member.id,
      token_hash: tokenHash
    });
    if (tokenError) {
    ...
    const welcomeCoffeeExpiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
    const { data: welcomeCoffee, error: welcomeCoffeeError } = await admin.from("kweider_welcome_coffees").insert({
      member_id: member.id,
      status: "available",
      expires_at: welcomeCoffeeExpiresAt
    }).select("id, status, issued_at, expires_at, redeemed_at, redeemed_by_name, purchase_amount, receipt_reference").single();
    if (welcomeCoffeeError || !welcomeCoffee) {
      console.error("Unable to create welcome coffee:", welcomeCoffeeError);
      throw new ApiError(500, "welcome_coffee_creation_failed", "The welcome coffee could not be prepared.");
    }
    const { data: welcome, error: welcomeError } = await admin.from("kweider_loyalty_transactions").insert({
      member_id: member.id,
    ...
    if (welcomeError || !welcome) {
      ...
      throw new ApiError(500, "welcome_transaction_failed", "The membership history could not be created.");
    }
    return json({
      ok: true,
...
const loginWithPin = async (payload, admin)=>{
  const phone = cleanText(payload.phone);
  const pin = validatePin(payload.pin);
  if (!phone) {
    throw new ApiError(400, "invalid_phone", "Enter your mobile number.");
  ...
  const normalisedPhone = await normaliseMemberPhone(admin, phone);
  const { data: member, error } = await admin.from("kweider_members").select(memberSelect).eq("phone_e164", normalisedPhone).maybeSingle();
  if (error) {
  ...
  const lockedUntilMs = member.access_pin_locked_until ? new Date(member.access_pin_locked_until).getTime() : 0;
  ...
  if (!constantTimeEqual(candidateHash, member.access_pin_hash)) {
    ...
    const lockedUntil = shouldLock ? new Date(Date.now() + PIN_LOCK_MINUTES * 60 * 1000).toISOString() : null;
    await admin.from("kweider_members").update({
...
const setMemberPin = async (payload, admin)=>{
  ...
  const tokenHash = await sha256Hex(token);
  const { data: accessToken, error: accessError } = await admin.from("kweider_member_access_tokens").select("id, member_id, revoked_at").eq("token_hash", tokenHash).maybeSingle();
  if (accessError || !accessToken || accessToken.revoked_at) {
    throw new ApiError(401, "invalid_card_token", "The saved membership card is no longer valid.");
  }
  const pinSalt = createPinSalt();
  const pinHash = await hashPin(pin, pinSalt);
  const { error: updateError } = await admin.from("kweider_members").update({
    access_pin_hash: pinHash,
    access_pin_salt: pinSalt,
    access_pin_failed_attempts: 0,
    access_pin_locked_until: null,
...
const openSavedCard = async (payload, admin)=>{
  const token = cleanText(payload.token);
  if (token.startsWith("rec_")) {
    throw new ApiError(401, "recovery_token_required", "Open the complete recovery link supplied by a member of staff.");
  }
  if (token.length < 30) {
    throw new ApiError(401, "invalid_card_token", "The saved membership card is not valid.");
  }
  ...
  await admin.from("kweider_member_access_tokens").update({
...
const recoverMemberCard = async (payload, admin)=>{
  ...
  if (!expiresAtMs || recoveryToken.length < 40) {
    throw new ApiError(401, "invalid_recovery_link", "This card recovery link is not valid.");
  }
  const recoveryHash = await sha256Hex(recoveryToken);
  const { data: recoveryAccess, error: recoveryError } = await admin.from("kweider_member_access_tokens").select("id, member_id, revoked_at").eq("token_hash", recoveryHash).maybeSingle();
  if (recoveryError || !recoveryAccess || recoveryAccess.revoked_at) {
    throw new ApiError(401, "recovery_link_used", "This card recovery link has already been used or cancelled.");
  }
  if (Date.now() > expiresAtMs) {
    await admin.from("kweider_member_access_tokens").update({
      revoked_at: new Date().toISOString()
    }).eq("id", recoveryAccess.id);
    throw new ApiError(410, "recovery_link_expired", "This card recovery link has expired. Ask a member of staff for a new link.");
  }
  ...
  const permanentToken = createRawToken();
  const permanentHash = await sha256Hex(permanentToken);
  const { data: permanentAccess, error: permanentError } = await admin.from("kweider_member_access_tokens").insert({
  ...
  if (permanentError || !permanentAccess) {
    ...
    throw new ApiError(500, "card_restore_failed", "The membership card could not be restored.");
  }
  const nowIso = new Date().toISOString();
  const { error: revokeError } = await admin.from("kweider_member_access_tokens").update({
    revoked_at: nowIso
  }).eq("member_id", recoveryAccess.member_id).neq("id", permanentAccess.id).is("revoked_at", null);
  if (revokeError) {
    console.error("Unable to revoke previous card tokens:", revokeError);
    await admin.from("kweider_member_access_tokens").delete().eq("id", permanentAccess.id);
    throw new ApiError(500, "card_restore_failed", "The membership card could not be restored safely.");
  }
  return json({
    ok: true,
    action: "recover_card",
...
const configureWebPush = ()=>{
  ...
  if (!subject || !publicKey || !privateKey) {
    throw new ApiError(503, "push_configuration_missing", "Reward notifications are temporarily unavailable.");
  }
  if (!vapidConfigured) {
...
const updateSubscriptionAfterPush = async (admin, subscription, succeeded, statusCode = 0)=>{
  const nowIso = new Date().toISOString();
  const expired = statusCode === 404 || statusCode === 410;
  const failureCount = succeeded ? 0 : Number(subscription.failure_count || 0) + 1;
  const update = {
    last_seen_at: nowIso,
    updated_at: nowIso,
  ...
  const { error } = await admin.from("kweider_push_subscriptions").update(update).eq("id", subscription.id);
  if (error) {
    console.error("Unable to update push subscription delivery state:", error);
  }
};
const sendPushPayload = async (admin, subscription, payload)=>{
  try {
    configureWebPush();
    ...
    console.error("Web Push delivery failed", {
      ...
      apnsId: cleanText(pushError?.headers?.["apns-id"]) || cleanText(pushError?.headers?.["apns-request-id"]) || null,
      contentType: cleanText(pushError?.headers?.["content-type"]) || null
    });
    await updateSubscriptionAfterPush(admin, subscription, false, statusCode);
    return false;
  }
};
const activePushSubscriptions = async (admin, memberId)=>{
  const { data, error } = await admin.from("kweider_push_subscriptions").select("id, endpoint, p256dh_key, auth_secret, failure_count").eq("member_id", memberId).eq("active", true);
  if (error) {
...
const sendPendingMessagesForMember = async (admin, memberId)=>{
  const { data: member, error: memberError } = await admin.from("kweider_members").select("notification_consent").eq("id", memberId).maybeSingle();
  ...
  const nowIso = new Date().toISOString();
  const { data: messages, error: messageError } = await admin.from("kweider_member_messages").select("id, title_en, body_en, dedupe_key, message_type, created_at").eq("member_id", memberId).eq("push_status", "pending").lte("not_before", nowIso).or(`expires_at.is.null,expires_at.gt.${nowIso}`).order("created_at", {
    ascending: true
  }).limit(10);
  if (messageError) {
  ...
  for (const message of messages){
    ...
    for (const subscription of subscriptions){
      const delivered = await sendPushPayload(admin, subscription, {
        ...
        body: cleanText(message.body_en) || "You have a new Kweider reward update.",
    ...
    const { error: updateError } = await admin.from("kweider_member_messages").update({
      push_status: messageDelivered ? "sent" : "failed",
      push_sent_at: messageDelivered ? deliveredAt : null
    }).eq("id", message.id).eq("push_status", "pending");
    if (updateError) {
...
const processPendingPushes = async (ctx)=>{
  if (ctx.authMode !== "secret") {
    throw new ApiError(403, "secret_auth_required", "This notification worker requires secret authentication.");
  }
  const nowIso = new Date().toISOString();
  const { error: inactivityError } = await ctx.supabaseAdmin.rpc("kweider_queue_inactivity_reminders", {
    p_now: nowIso
  });
  if (inactivityError) {
    console.error("Unable to queue inactivity reminders:", inactivityError);
  }
  const { data: pendingRows, error } = await ctx.supabaseAdmin.from("kweider_member_messages").select("member_id").eq("push_status", "pending").lte("not_before", nowIso).or(`expires_at.is.null,expires_at.gt.${nowIso}`).limit(500);
  if (error) {
    throw new ApiError(500, "pending_push_query_failed", "Pending reward notifications could not be loaded.");
  }
  const memberIds = [
    ...new Set((pendingRows ?? []).map((row)=>row.member_id))
  ];
  let sent = 0;
  let failed = 0;
  let pending = 0;
  for (const memberId of memberIds){
    const result = await sendPendingMessagesForMember(ctx.supabaseAdmin, String(memberId));
    sent += result.sent;
...
const updateNotificationPreference = async (payload, admin)=>{
  ...
  if (memberError) {
    ...
    throw new ApiError(500, "notification_preference_failed", "The notification preference could not be saved.");
  }
  if (!enabled) {
    const { error: subscriptionError } = await admin.from("kweider_push_subscriptions").update({
      active: false,
      revoked_at: nowIso,
    ...
    if (subscriptionError) {
      ...
      throw new ApiError(500, "notification_disable_failed", "Notifications could not be turned off safely.");
    }
  }
  await admin.from("kweider_member_access_tokens").update({
    last_used_at: nowIso
...
const savePushSubscription = async (payload, admin)=>{
  const { accessToken, memberId } = await requireMemberAccess(admin, payload.token);
  ...
  const { data: member, error: memberError } = await admin.from("kweider_members").select("id, status, notification_consent").eq("id", memberId).maybeSingle();
  if (memberError || !member) {
    throw new ApiError(404, "member_not_found", "The membership could not be found.");
  }
  if (member.status !== "active") {
  ...
  if (member.notification_consent !== true) {
    throw new ApiError(409, "notification_consent_required", "Enable reward notifications before registering this device.");
  }
  const nowIso = new Date().toISOString();
  const { data: savedSubscription, error: subscriptionError } = await admin.from("kweider_push_subscriptions").upsert({
    member_id: memberId,
    endpoint,
    p256dh_key: p256dh,
    auth_secret: auth,
    ...
    onConflict: "endpoint"
  }).select("id, endpoint, p256dh_key, auth_secret, failure_count").single();
  if (subscriptionError || !savedSubscription) {
    console.error("Unable to save push subscription:", subscriptionError);
    throw new ApiError(500, "push_subscription_failed", "Notifications could not be enabled on this device.");
  }
  const testNotificationSent = await sendPushPayload(admin, savedSubscription, {
...
const removePushSubscription = async (payload, admin)=>{
  const { accessToken, memberId } = await requireMemberAccess(admin, payload.token);
  const endpoint = cleanText(payload.pushEndpoint);
  if (!endpoint) {
    throw new ApiError(400, "invalid_push_endpoint", "The notification subscription is not valid.");
  ...
  const nowIso = new Date().toISOString();
  const { error } = await admin.from("kweider_push_subscriptions").update({
    active: false,
    revoked_at: nowIso,
    last_seen_at: nowIso,
    disabled_reason: "device_opt_out",
    updated_at: nowIso
  }).eq("member_id", memberId).eq("endpoint", endpoint);
  if (error) {
    console.error("Unable to remove push subscription:", error);
    throw new ApiError(500, "push_unsubscribe_failed", "Notifications could not be turned off on this device.");
  }
  await admin.from("kweider_member_access_tokens").update({
    last_used_at: nowIso
  }).eq("id", accessToken.id);
...
const queueNearRewardMessage = async (admin, bundle)=>{
  ...
  ].sort((left, right)=>Number(left.thresholdPoints) - Number(right.thresholdPoints)) : [];
  const nextDefinition = definitions.find((definition)=>Number(definition.thresholdPoints) > points);
  ...
  const { count, error: countError } = await admin.from("kweider_member_messages").select("id", {
    count: "exact",
    head: true
  }).eq("member_id", member.id).in("message_type", [
    "near_reward",
    "inactivity"
...
const markMessagesRead = async (payload, admin)=>{
  const token = cleanText(payload.token);
  if (token.length < 30) {
    throw new ApiError(401, "invalid_card_token", "The saved membership card is not valid.");
  }
  ...
  const { error: updateError } = await admin.from("kweider_member_messages").update({
    is_read: true,
    read_at: nowIso
  }).eq("member_id", accessToken.member_id).eq("is_read", false);
  if (updateError) {
    console.error("Unable to mark messages as read:", updateError);
    throw new ApiError(500, "messages_update_failed", "The messages could not be updated.");
  }
  await admin.from("kweider_member_access_tokens").update({
    last_used_at: nowIso
  }).eq("id", accessToken.id);
...
const requireStaff = async (req, ctx)=>{
  const authorization = cleanText(req.headers.get("Authorization"));
  const match = authorization.match(/^Bearer\s+(.+)$/i);
  const accessToken = match?.[1]?.trim() ?? "";
  if (!accessToken) {
    throw new ApiError(401, "staff_login_required", "Sign in with an authorised staff account.");
  }
  const { data: { user }, error: userError } = await ctx.supabaseAdmin.auth.getUser(accessToken);
  if (userError || !user || !isUuid(user.id)) {
    throw new ApiError(401, "invalid_staff_session", "The staff session is invalid or has expired.");
  }
  const userId = user.id;
  const { data: profile, error } = await ctx.supabaseAdmin.from("kweider_staff_profiles").select("user_id, display_name, staff_role, active").eq("user_id", userId).maybeSingle();
  if (error) {
    console.error("Unable to read staff profile:", error);
    throw new ApiError(500, "staff_profile_failed", "The staff account could not be checked.");
  }
  if (!profile || !profile.active) {
    throw new ApiError(403, "staff_not_authorised", "This account is not authorised for rewards operations.");
  }
  return {
    userId,
...
const findMemberForStaff = async (payload, ctx, staff)=>{
  const search = cleanText(payload.memberSearch);
  if (!search || search.length > 100) {
    throw new ApiError(400, "invalid_member_search", "Enter a membership code or phone number.");
  }
  let member = null;
  if (/^KW-/i.test(search)) {
    const { data, error } = await ctx.supabaseAdmin.from("kweider_members").select(memberSelect).eq("member_code", search.toUpperCase()).maybeSingle();
    if (error) {
    ...
    const { data: normalisedPhone, error: phoneError } = await ctx.supabaseAdmin.rpc("kweider_normalize_phone", {
      p_phone: search
    });
    if (phoneError || !normalisedPhone) {
      throw new ApiError(400, "invalid_member_search", "Enter a valid membership code or phone number.");
    }
    const { data, error } = await ctx.supabaseAdmin.from("kweider_members").select(memberSelect).eq("phone_e164", String(normalisedPhone)).maybeSingle();
    if (error) {
  ...
  const pinResetRequest = await latestActivePinReset(ctx.supabaseAdmin, member.id);
  return json({
    ok: true,
...
const issueMemberRecoveryToken = async (payload, ctx, staff)=>{
  const memberId = cleanText(payload.memberId);
  if (!isUuid(memberId)) {
    throw new ApiError(400, "invalid_member_id", "Select a valid member.");
  }
  const { data: member, error: memberError } = await ctx.supabaseAdmin.from("kweider_members").select("id, member_code, first_name, status").eq("id", memberId).maybeSingle();
  if (memberError) {
    console.error("Unable to check member for recovery:", memberError);
    throw new ApiError(500, "member_recovery_check_failed", "The membership could not be checked.");
  }
  if (!member) {
    throw new ApiError(404, "member_not_found", "The membership could not be found.");
  }
  ...
  const tokenHash = await sha256Hex(rawToken);
  const { error: tokenError } = await ctx.supabaseAdmin.from("kweider_member_access_tokens").insert({
    member_id: memberId,
    token_hash: tokenHash
  });
  if (tokenError) {
    console.error("Unable to create card recovery token:", tokenError);
    throw new ApiError(500, "recovery_link_failed", "The card recovery link could not be created.");
  }
...
const approveMemberPinReset = async (payload, ctx, staff)=>{
  const memberId = cleanText(payload.memberId);
  if (!isUuid(memberId)) {
    throw new ApiError(400, "invalid_member_id", "Select a valid member.");
  }
  const { data: member, error: memberError } = await ctx.supabaseAdmin.from("kweider_members").select("id, status").eq("id", memberId).maybeSingle();
  if (memberError || !member) {
    throw new ApiError(404, "member_not_found", "The membership could not be found.");
  }
  if (member.status !== "active") {
  ...
  const { data: approved, error: approveError } = await ctx.supabaseAdmin.from("kweider_pin_reset_requests").update({
    status: "approved",
    approved_at: new Date().toISOString(),
    approved_by: staff.userId
  }).eq("id", request.id).eq("status", "pending").select("id, status, requested_at, expires_at, approved_at").single();
...
const rpcApiError = (error)=>{
  ...
  const mappings = [
    ...
    [
      ...
      403,
      "staff_not_authorised",
      "This staff account is not authorised."
    ],
    [
      "MEMBER_REQUIRED",
      400,
    ...
    [
      "PURCHASE_TOO_SMALL_FOR_POINTS",
      400,
      "purchase_too_small",
      "This purchase does not earn a whole point."
    ],
    [
      "INVALID_RECEIPT_REFERENCE",
      400,
      "invalid_receipt_reference",
      "Enter a valid receipt reference."
    ],
    [
      "INVALID_IDEMPOTENCY_KEY",
      400,
      "invalid_request_key",
      "The operation reference is invalid."
    ],
    [
      "IDEMPOTENCY_KEY_CONFLICT",
      409,
      "request_key_conflict",
      "This operation reference was already used for another transaction."
    ],
    [
      "kweider_loyalty_unique_earn_receipt",
      409,
      "receipt_already_used",
      "This receipt has already been used."
    ],
    [
      "LOYALTY_SETTINGS_UNAVAILABLE",
      500,
      "settings_unavailable",
      "The loyalty settings are unavailable."
    ],
    [
    ...
    [
      "UNSUPPORTED_REWARD_KIND",
      400,
      "unsupported_reward_kind",
      "This reward type is not supported."
    ],
    [
      "BREAKFAST_CONFIRMATION_REQUIRED",
      400,
      "breakfast_confirmation_required",
    ...
    [
      ...
      "This checkout reference was already used for different details."
    ]
  ];
  ...
  return new ApiError(500, "rewards_operation_failed", "The rewards operation could not be completed.");
};
const addPurchasePoints = async (payload, ctx, staff)=>{
  const memberId = cleanText(payload.memberId);
  ...
  if (!Number.isFinite(purchaseAmount)) {
    throw new ApiError(400, "invalid_purchase_amount", "Enter a valid purchase amount.");
  }
  const { data, error } = await ctx.supabaseAdmin.rpc("kweider_staff_add_purchase_points", {
  ...
  const pushDelivery = await sendPendingMessagesForMember(ctx.supabaseAdmin, memberId);
  return json({
    ok: true,
...
const redeemMemberReward = async (payload, ctx, staff)=>{
  const memberId = cleanText(payload.memberId);
  const memberRewardId = cleanText(payload.memberRewardId);
  const receiptReference = cleanText(payload.receiptReference);
  const idempotencyKey = cleanText(payload.idempotencyKey);
  ...
  if (!isUuid(memberRewardId)) {
    throw new ApiError(400, "invalid_member_reward_id", "Choose a valid available reward.");
  }
  if (purchaseAmount !== null && !Number.isFinite(purchaseAmount)) {
    throw new ApiError(400, "invalid_purchase_amount", "Enter a valid purchase amount.");
  }
  const { data, error } = await ctx.supabaseAdmin.rpc("kweider_staff_redeem_member_reward", {
    p_staff_user_id: staff.userId,
    p_member_id: memberId,
...
const completeCheckout = async (payload, ctx, staff)=>{
  const memberId = cleanText(payload.memberId);
  const rawPurchaseAmount = payload.purchaseAmount;
  ...
  if (!Number.isFinite(purchaseAmount) || purchaseAmount < 0) {
    throw new ApiError(400, "invalid_purchase_amount", "Enter a valid bill amount.");
  }
  if (selectedMemberRewardId && !isUuid(selectedMemberRewardId)) {
    throw new ApiError(400, "invalid_member_reward_id", "The reward selected by the customer is not valid.");
  }
  if (!idempotencyKey || idempotencyKey.length > 200) {
    throw new ApiError(400, "invalid_request_key", "The checkout reference is invalid.");
  }
  const { data, error } = await ctx.supabaseAdmin.rpc("kweider_staff_complete_checkout", {
    p_staff_user_id: staff.userId,
    p_member_id: memberId,
    p_purchase_amount: purchaseAmount,
    p_idempotency_key: idempotencyKey,
    p_selected_member_reward_id: selectedMemberRewardId || null,
  ...
  let welcomeCoffeeRedemption = null;
  let welcomeCoffeeWarning = "";
  if (welcomeCoffeeConfirmed) {
    const { data: coffeeData, error: coffeeError } = await ctx.supabaseAdmin.rpc("kweider_redeem_welcome_coffee", {
      p_staff_user_id: staff.userId,
      p_member_id: memberId,
      p_purchase_amount: purchaseAmount,
      p_receipt_reference: receiptReference || null,
    ...
    if (coffeeError) {
      console.error("Checkout completed but welcome coffee was not marked:", coffeeError);
      welcomeCoffeeWarning = "Checkout completed, but the welcome coffee was not marked as used. Ask a manager to review it.";
    } else {
      welcomeCoffeeRedemption = coffeeData && typeof coffeeData === "object" ? coffeeData : null;
    }
  }
  let bundle = await loadMemberBundle(ctx.supabaseAdmin, memberId);
  const reminderQueued = await queueNearRewardMessage(ctx.supabaseAdmin, bundle);
...
const rewardsApiFetch = withSupabase({
  ...
  try {
    ...
    if (action === "staff_me" || action === "staff_find_member" || action === "staff_issue_recovery_token" || action === "staff_approve_pin_reset" || action === "staff_complete_checkout" || action === "staff_add_purchase_points" || action === "staff_redeem_member_reward") {
    ...
    throw new ApiError(400, "unknown_action", "The requested action is not supported.");
  } catch (error) {
